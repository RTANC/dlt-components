  ...
  const [loading, setLoading] = useState(false)
    ...
    const search = async () => {
        try {
          ...
          const report = await jsreport.render({ template: { shortid: 'lldYmhM5F3' } })
          report.openInWindow({ title: 'GCS02', filename: 'GCS02.pdf' })
        } catch (error) {
          console.log(error)
        } finally {
          setLoading(false)
        ...
        <Container sx={{height: '100vh'}}>
          <Stack spacing={2}>
            <div className='dlt-page-title'>
              <div className='dlt-page-title-text'>GCS02 จำนวนรถและปริมาณสินค้าที่เข้า-ออกสถานี ตามประเภทรถ</div>
