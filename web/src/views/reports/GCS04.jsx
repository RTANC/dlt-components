  ...
  const [loading, setLoading] = useState(false)
    ...
    const search = async () => {
        try {
          ...
          const report = await jsreport.render({ template: { shortid: 'H-Uyo4wBRX' } })
          report.openInWindow({ title: 'GCS04', filename: 'GCS04.pdf' })
        } catch (error) {
          console.log(error)
        } finally {
          setLoading(false)
        ...
        <Container sx={{height: '100vh'}}>
          <Stack spacing={2}>
            <div className='dlt-page-title'>
              <div className='dlt-page-title-text' style={{fontSize: 32}}>GCS04 จำนวนรถและปริมาณสินค้าที่เข้า-ออกสถานี ตามภูมิภาค ต้นทาง-ปลายทาง</div>
