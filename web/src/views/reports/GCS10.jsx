  ...
  const [loading, setLoading] = useState(false)
    ...
    const search = async () => {
        try {
          ...
          const report = await jsreport.render({ template: { shortid: 'VWfwYFJGWl' } })
          report.openInWindow({ title: 'GCS10', filename: 'GCS10.pdf' })
        } catch (error) {
          console.log(error)
        } finally {
          setLoading(false)
        ...
        <Container sx={{height: '100vh'}}>
          <Stack spacing={2}>
            <div className='dlt-page-title'>
              <div className='dlt-page-title-text'>GCS10 ระยะเวลาเฉลี่ยที่รถแต่ละคันใช้เวลาอยู่ในสถานี แยกตามประเภทรถ</div>
