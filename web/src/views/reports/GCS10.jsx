...
export default function GCS10() {
  const [loading, setLoading] = useState(false)
    ...
    const search = async () => {
        try {
          setLoading(true)
          const fileURL = await getReports('10', getKeyValue(query))
          window.open(fileURL)
        } catch (error) {
          console.log(error)
        } finally {
        ...
        <Container>
          <Stack spacing={2}>
            <div className='dlt-page-title'>
              <div className='dlt-page-title-text-vl-2'>GCS10 ระยะเวลาเฉลี่ยที่รถแต่ละคันใช้เวลาอยู่ในสถานี แยกตามประเภทรถ</div>
