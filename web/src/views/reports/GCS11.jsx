  ...
  const [loading, setLoading] = useState(false)
    ...
    const search = async () => {
        try {
          ...
          const report = await jsreport.render({ template: { shortid: 'lmeMkheHI' } })
          report.openInWindow({ title: 'GCS11', filename: 'GCS11.pdf' })
        } catch (error) {
          console.log(error)
        } finally {
          setLoading(false)
        ...
        <Container sx={{height: '100vh'}}>
          <Stack spacing={2}>
            <div className='dlt-page-title'>
              <div className='dlt-page-title-text'>GCS11 ระยะเวลาเฉลี่ยที่รถแต่ละคันใช้เวลาอยู่ในสถานี แยกตามผู้ประกอบการ</div>
