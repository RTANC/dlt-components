  ...
  const [loading, setLoading] = useState(false)
    ...
    const search = async () => {
        try {
          ...
          const report = await jsreport.render({ template: { shortid: 'D3B-iDkJvW' } })
          report.openInWindow({ title: 'GCS15', filename: 'GCS15.pdf' })
        } catch (error) {
          console.log(error)
        } finally {
          setLoading(false)
        ...
        <Container sx={{height: '100vh'}}>
          <Stack spacing={2}>
            <div className='dlt-page-title'>
              <div className='dlt-page-title-text'>GCS15 จำนวนรถรับส่งสินค้า แยกตามประเภทสินค้า</div>
